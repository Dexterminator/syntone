...
function setCallbacks (socket) {
	socket.on('disconnect', function () {connected--; console.log('user disconnected')});
	socket.on('test message', function (message) {onTestMessage(socket, message)});
  socket.on('slider change', function (message) {onSliderChange(socket, message)});
}

function assignRoom (io, socket) {
	var joined = false;
	_.forEach(rooms, function (room) {
		if (roomSockets(io, room).length < SOCKETS_PER_ROOM) {
	...
	if (!joined) {
		roomIdCounter++;
		var room = roomIdCounter.toString();
		console.log('Rooms full, create and join room ' + room);
		rooms.push(room);
		socket.room = room;
		socket.join(room);
	};
